...
def runsim(generations, popsize, seed, numberofchromosomes, chromosomesize, Ub, Sb, Ud, Sd, numberofgenes, Ubname, Sbname, popsizename, generationsname, chromsizename, Udname, Sdname, seedname):
	...
	#This is a constant to describe how much of the genome will be covered with genes. In humans, this number is 0.01 for genes, but I choose a higher number to account for conserved non-coding regions etc.
	fractionofgenomecoding = 0.1

	#Genes will be evenly distributed, so this block length is the length of a gene plus a proportional amount of non-coding genome, depending on the fraction of the genome which is coding.
	geneblocklength = genomesize / numberofgenes
	genelength = geneblocklength * fractionofgenomecoding
	
	#This should produce a list of recombination intervals to put into recregions later.
	chromosomelist = []	
	for i in range(0, numberofchromosomes-1):
		chromosomelist.append(fwdpy11.FixedCrossovers(i*chromosomesize, (i*chromosomesize + chromosomesize), 2))
	
	genelist = []
	for i in range(0, numberofgenes-1):
		genelist.append(fwdpy11.ConstantS(i*geneblocklength, i*geneblocklength + genelength, 1, Sd, 0.5))
...
if __name__ == "__main__":
	...
	Sb = float(sys.argv[8])
	numberofgenes = int(sys.argv[9])
	typeofrun = sys.argv[10]
	slopeforcontourline = float(sys.argv[11])
	randomnumberseed = int(sys.argv[12])
	beneficialson = bool(sys.argv[13])
	...
	if typeofrun == 'single':
		if beneficialson:
			directoryname = 'dataforUb' + sys.argv[7] + 'Sb' + sys.argv[8] + 'popsize' + sys.argv[2] + 'chromosomesize' + sys.argv[5] + 'Ud' + sys.argv[3] + 'Sd' + sys.argv[4] + 'numberofgenes' + sys.argv[9] + 'seed' + sys.argv[12]
			os.mkdir(directoryname)
			os.chdir(directoryname)
		else:
			directoryname = 'datafornobeneficialspopsize' + sys.argv[2] + 'chromosomesize' + sys.argv[5] + 'Ud' + sys.argv[3] + 'Sd' + sys.argv[4] + 'numberofgenes' + sys.argv[9] + 'seed' + sys.argv[12]
			os.mkdir(directoryname)
			os.chdir(directoryname)
		
		#Now that we're in the right directory, just run the simulation
		runsim(generations, popsize, randomnumberseed, numberofchromosomes, chromosomesize, beneficialmutationrate, Sb, deleteriousmutationrate, Sd, numberofgenes, sys.argv[7], sys.argv[8], sys.argv[2], sys.argv[1], sys.argv[5], sys.argv[3], sys.argv[4], sys.argv[12])
		
	elif typeofrun == 'root':
		if beneficialson:
			directoryname = 'dataforUb' + sys.argv[7] + 'slope' + sys.argv[11] + 'popsize' + sys.argv[2] + 'chromosomesize' + sys.argv[5] + 'numberofgenes' + sys.argv[9] + 'seed' + sys.argv[12]
			os.mkdir(directoryname)
			os.chdir(directoryname)
		else:
			directoryname = 'datafornobeneficialsslope' + sys.argv[11] + 'popsize' + sys.argv[2] + 'chromosomesize' + sys.argv[5] + 'numberofgenes' + sys.argv[9] + 'seed' + sys.argv[12]
			os.mkdir(directoryname)
			os.chdir(directoryname)
		...
		bracketedSbs = BracketRootforSb(generations, popsize, randomnumberseed, numberofchromosomes, chromosomesize, slopeforcontourline, beneficialmutationrate, Sb1, Sb2, deleteriousmutationrate, Sd, sys.argv[7], Sb1name, Sb2name, sys.argv[2], sys.argv[1], sys.argv[5], sys.argv[3], sys.argv[4], sys.argv[12], miscfile)
		Sb1 = bracketedSbs[0]
		Sb2 = bracketedSbs[1]
		
		#Find an Sb value that produces desired slope.
		rootSb = FindingSbValueWithGivenSlope(generations, popsize, randomnumberseed, numberofchromosomes, chromosomesize, slopeforcontourline, beneficialmutationrate, Sb1, Sb2, deleteriousmutationrate, sys.argv[7], Sb1name, Sb2name, sys.argv[2], sys.argv[1], sys.argv[5], sys.argv[3], sys.argv[4], sys.argv[12], miscfile)
